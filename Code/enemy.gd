...
func _on_body_entered(body: Node2D) -> void:
	print("collision") # for testing
	
	# if the player hits the enemy, kill the enemy
	if body.name == "Player" and body.alive and Input.is_action_just_pressed("attack"):
...
# todo
# fix animations not playing and any other collision issues
