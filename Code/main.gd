...
#first load is used to check if this is the first time the game loads a level
func _load_level(level_number, first_load) -> void:

# if there is a current level loaded, unload it
	if current_level_root:
		
		#disconnect active signals if it isn't the first time a level uis being loaded
		if!first_load:
			call_deferred("_unset_level", current_level_root)
	...
	call_deferred("_setup_level", current_level_root)
	
	if !first_load:
		# fade in the new level
		await fade.fade(0.0, 1.5).finished
		
		print(current_level_root.get_node_or_null("Player"))
...
#disconnecting the signals once they are no longer needed
func _unset_level(level_root):
	
	# disconnecting the enemies signals
	var enemies = level_root.get_node_or_null("Enemies")
	
	# disconnecting the player hit signal 
	if enemies:
		for enemy in enemies.get_children():
			enemy.player_hit.disconnect(_on_player_hit)
	
	#disconnecting the exit signal
	var exit = level_root.get_node_or_null("Exit")
	exit.body_entered.disconnect(_on_exit_body_entered)
...
func _on_button_pressed() -> void:
	...
	var level_root = current_level_root
